...
class ImportantFeaturesFilter(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        return self
    def transform(self, X):
        idx = np.asarray(np.argwhere(self.feature_importances > 0.01).flatten())
        return X[:, idx]
...
full_pipeline2 = Pipeline([
        ...
        ("important_features_pipeline", important_features_pipeline),
    ])
...
param_grid = [
    {'n_estimators': [3, 10, 30], 'max_features': [2, 3, 4, 6, 7, 8]},
    {'bootstrap': [False], 'n_estimators': [3, 10], 'max_features': [2, 3, 4, 6, 7, 8]},
  ]
